import { DEFAULT_KCOLOR, ZERO_VECTOR } from "../constants";
...
function drawRemainingItems() {
  ...
  for (let i = 0; i < remainingStarters.length; i++) {
    ...
    g.season8.starterSprites[i].Render(pos, ZERO_VECTOR, ZERO_VECTOR);
  }
  y = y - yAdjustment + 20;
  ...
  for (let i = 0; i < remainingGoodDevilItems.length; i++) {
    ...
    x += 22 * scale;
    const pos = Vector(x, y);
    g.season8.devilSprites[i].Render(pos, ZERO_VECTOR, ZERO_VECTOR);
  }
  y = y - yAdjustment + 20;
  ...
  for (let i = 0; i < remainingGoodAngelItems.length; i++) {
    ...
    x += 22 * scale;
    const pos = Vector(x, y);
    g.season8.angelSprites[i].Render(pos, ZERO_VECTOR, ZERO_VECTOR);
  }
}
