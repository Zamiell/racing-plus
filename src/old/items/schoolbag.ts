import { ALL_CACHE_FLAGS_MINUS_FAMILIARS, ZERO_VECTOR } from "../constants";
...
export function spriteDisplay(): void {
  ...
  // Draw the item image
  sprites.item.Update();
  sprites.item.Render(itemVector, ZERO_VECTOR, ZERO_VECTOR);
  ...
  if (maxCharges !== 0) {
    ...
    sprites.barMeter.Render(barVector, Vector(0, meterClip1), ZERO_VECTOR);
