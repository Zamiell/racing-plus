...
export function main(): void {
  ...
  if (gameFrameCount === 0 && !g.run.reseededFloor) {
    return;
  }
  if (g.run.reseededFloor) {
...
function newLevel() {
  ...
  if (
    ...
    stage === 11 &&
    stageType === 0 && // 11.0 is Dark Room
    // Check for Forget Me Now
    g.run.level.stage !== 10 &&
    g.run.level.stage !== 11
  ) {
    // We arrived at the Dark Room without going through Sheol
    Isaac.DebugString("Sacrifice Room teleport detected.");
    fastTravel.trapdoor.gotoNextFloor(false, g.run.level.stage + 1);
  ...
  if (
    (challenge !== 0 || !customRun) && // Disable all reseeding for set seeds
    stage !== 12 // Disable all reseeding on The Void
  ) {
    if (
      checkDualityNarrowRoom() || // Check for Duality restrictions
      checkForgottenSoftlock() || // Forgotten can become softlocked in certain rooms
      checkDupeRooms() // Check for duplicate rooms
      // (checking for duplicate rooms has to be the last check because it will store the rooms as
      // "seen")
    ) {
      g.run.reseededFloor = true;
      g.run.level.reseedCount += 1;
      if (g.p.GetEternalHearts() === 1) {
  ...
  Isaac.DebugString(`New floor: ${g.run.level.stage}-${g.run.level.stageType}`);

  // Clear variables that track things per level
  Isaac.DebugString(`Reseed count: ${g.run.level.reseedCount}`);
