 ...
 * It Lives!
 */
export class SubvertTeleport extends ConfigurableModFeature {
  configKey: keyof Config = "SubvertTeleport";

  @CallbackCustom(ModCallbackCustom.POST_NEW_ROOM_REORDERED)
  postNewRoomReordered(): void {
    if (this.shouldSubvertTeleport()) {
      this.subvertTeleport();
    }
  }

  shouldSubvertTeleport(): boolean {
  ...
  subvertTeleport(): void {
    const doorSlot = this.getRoomEnterDoorSlot();
    const position = getDoorSlotEnterPosition(doorSlot);

    for (const player of getAllPlayers()) {
      player.Position = position;

      // If we are The Soul, the Forgotten body will also need to be teleported. However, if we
      // change its position manually, it will just warp back to the same spot on the next frame.
   ...
   *   1x1 room.)
   */
  getRoomEnterDoorSlot(): OneByOneRoomDoorSlot {
