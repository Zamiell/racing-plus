...
export function fastClearPostNPCUpdate(npc: EntityNPC): void {
  ...
  if (npc.HasEntityFlags(EntityFlag.FRIENDLY)) {
    trackingRemove.fastClearCheckRemove(npc, false, "NPC_UPDATE_FLAG_FRIENDLY");
    return;
  }
  ...
  if (isDyingEggyWithNoSpidersLeft(npc)) {
    trackingRemove.fastClearCheckRemove(npc, false, "NPC_UPDATE_DYING_EGGY");
    return;
  }
  ...
  // they are dead on every frame.
  if (isDyingDump(npc)) {
    trackingRemove.fastClearCheckRemove(npc, false, "NPC_UPDATE_DYING_DUMP");
    return;
  }
...
function checkAdd(entity: Entity, parentCallback: string) {
  ...
  // We do not care if the NPC is already dead. (This is needed because we can enter this function
  // from the `POST_NPC_UPDATE` callback.)
  if (entity.IsDead()) {
    return;
  }

  // We do not care if this is a specific child NPC attached to some other NPC (like Death's
  // scythes).
