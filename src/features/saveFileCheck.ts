...
const SAVE_FILE_SEED = "31XY AQGT"; // cspell:disable-line
const EDEN_ACTIVE_ITEM = CollectibleType.COLLECTIBLE_DEATH_CERTIFICATE;
const EDEN_PASSIVE_ITEM = CollectibleType.COLLECTIBLE_MR_DOLLY;
...
export function isNotFullyUnlocked(): boolean {
  ...
  if (g.saveFile.state === SaveFileState.GOING_BACK) {
    ...
    if (customRun !== g.saveFile.oldRun.seededRun) {
      valid = false;
    }
    if (
